...
window.desenharCardsReservas = function(reservas) {
    ...
    container.innerHTML = reservas.map(res => {
        ...
        const statusClass = isConfirmada ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700';
        const mesaStr = Array.isArray(res.mesas) ? res.mesas.join(', ') : String(res.mesas || '');
        ...
        // BOTÕES: Responsivos (flex-1 no mobile para esticarem juntos, tamanho fixo no PC)
        ...
        const botaoComprovante = res.comprovante_url ? `
            <button onclick="window.verComprovante('${res.comprovante_url}')" class="flex-1 sm:flex-none w-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 font-black text-[9px] uppercase tracking-widest py-2.5 sm:py-2 px-1 rounded-lg transition-all text-center">
                Ver PIX
            </button>
        ` : '';

        return `
        <div class="bg-white dark:bg-slate-900 p-4 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 md:gap-4 animate-fade-in shadow-sm hover:shadow-md transition-shadow">
            ...
            <div class="flex-1 min-w-0 w-full">
                ...
                ${telefoneInfo}
                <div class="mt-2 md:mt-3 bg-slate-50 dark:bg-slate-800/50 inline-block px-3 py-1.5 rounded-lg border border-slate-100 dark:border-slate-800">
                    <p class="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-wide">Mesa(s): <span class="text-emerald-500 font-black text-sm md:text-base ml-1">${mesaStr}</span></p>
                </div>
                ${res.tipo ? `<p class="text-[8px] md:text-[9px] font-black text-indigo-500 uppercase mt-2 tracking-wider">${res.tipo}</p>` : ''}
            </div>
            
            <div class="flex flex-row sm:flex-col gap-2 w-full sm:w-[90px] border-t sm:border-t-0 sm:border-l border-slate-100 dark:border-slate-800 pt-3 sm:pt-0 sm:pl-3 mt-2 sm:mt-0 shrink-0">
                ${botaoAprovar}
                ${botaoCancelar}
                ${botaoComprovante}
            </div>
